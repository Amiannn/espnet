...
class TrieProcessor():
    ...
    def _get_cache_key(self, element):
        return " ".join([str(e) for e in element])

    def build_trie(self, elements_list):
        tree  = {}
        cache = {}
        for elements in elements_list:
            ...
            for element in elements:
                ...
                now = now[element]
                childs.append(now)
            key = self._get_cache_key(elements)
            cache[key] = childs
    ...
    def build_batch_trie(self, uttblist):
        ...
        trieblist   = drouped_uttblist + globalblist
        tree, cache = self.build_trie(trieblist)

        return trieblist, tree, cache
...
if __name__ == '__main__':
    ...
    batch_data = [{'text': np.array([465,  79, 125,   3,  84, 240,  13,   5, 364,  27, 347,  30, 317,
       ...
       [18, 22]])}]
    tokenizer_config = {
        'token_type': 'bpe', 'bpemodel': 'data/en_token_list/bpe_unigram600/bpe.model', 'delimiter': None, 'space_symbol': '<space>', 'non_linguistic_symbols': None, 'g2p_type': None, 'nonsplit_symbol': None, 'tokenizer_language': 'en',
    ...
    trie_search = TrieProcessor(blist_path, 0.5, **tokenizer_config)
