...
class ESPnetBiasingASRModel(ESPnetASRModel):
    def __init__(
        self,
        vocab_size: int,
        ...
        rareword: Optional[torch.nn.Module],
        rareword_conf: dict = None,
        aux_ctc: dict = None,
        ...
        self.use_rareword_methods = True if self.rareword != None else False
    ...
    def _calc_transducer_loss(
        ...
        if self.use_rareword_methods and self.epoch >= self.rareword_conf['warmup_epoch']:
            (
                ptr_dist, 
                h_ptr, 
                dbias, 
            ) = self.rareword.forward_attention(
                encoder_out, 
                decoder_in,
                self.decoder.embed,
                self.decoder.dropout_embed, 
                masks_mat,
            )
            if self.rareword_conf['deepbiasing']:
            ...
            gate       = self.rareword.forward_gate(h_joint, h_ptr, masks_gate_mat)
            model_dist = torch.nn.functional.softmax(joint_out, dim=-1)
            joint_out  = self.rareword.forward_copy_mechanism(model_dist, ptr_dist, gate)
