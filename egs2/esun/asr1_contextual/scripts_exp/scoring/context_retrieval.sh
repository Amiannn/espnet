...
for folder in "${exp_folder[@]}"; do
    ...
    precision=$(echo "$output" | grep "^Macro-Averaged Precision at threshold 0.5:" | awk '{print $NF}')
    recall=$(echo "$output" | grep "Macro-Averaged Recall at threshold 0.5:" | awk '{print $NF}')
    f1=$(echo "$output" | grep "Macro-Averaged F1 Score at threshold 0.5:" | awk '{print $NF}')
    roc_auc=$(echo "$output" | grep "Overall ROC AUC:" | awk '{print $NF}')
