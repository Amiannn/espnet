...
retriever_pretrained_model=./exp/asr_whisper/run_medium_xdotproduct_contextual_retriever_balanced_alpha0.8_suffix/valid.loss.ave_10best.pth
adapter_pretrained_model=./exp/asr_whisper/run_medium_contextual_adapter_decoder_pretrained_ce/valid.loss.ave_10best_fixed.pth
...
CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=0 ./asr.sh \
    ...
    --inference_asr_model 0epoch_fixed.pth \
    --train_set "${train_set}" \
    --valid_set "${valid_set}" \
    --test_sets "${test_sets}" \
