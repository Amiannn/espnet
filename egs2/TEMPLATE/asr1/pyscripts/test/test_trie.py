...
def encode_entity(ent_list, tokenizer, idconverter):
    ent_datas = []
    for ent in ent_list:
        tokens = tokenizer.text2tokens(ent)
        ids    = idconverter.tokens2ids_withoutprompt(tokens)
        ent_datas.append(ids)
    return ent_datas

def tensorify(Xs, pad_value=-1):
...
if __name__ == '__main__':
    ...
    context_path = "./local/contextual/rarewords/esun_earningcall.entity.txt"
    ...
    ent_datas = [d[0] for d in read_file(context_path, sp='\t')]
    ent_datas.extend(["擴大我們", "corporate", "擴大家"])
    
    tokens    = tokenizer.text2tokens(text)
    token_ids = idconverter.tokens2ids(tokens)
    token_ids = torch.tensor(token_ids, dtype=torch.long)

    ent_ids    = encode_entity(ent_datas, tokenizer, idconverter)
    ent_ids_tensors, _ = tensorify(ent_ids)
    print(f'ent_ids: {ent_ids}')

    trie_processor = TrieProcessor(token_id_converter=idconverter)

    # Build the trie and cache
    trie = trie_processor.build_trie(ent_ids)
    # print(f'trie:\n{trie}')

    # collapse2token_idx = map_tokens_to_words(token_ids, tokens, text, tokenizer, idconverter)
    # print(f'collapse2token_idx: {collapse2token_idx}')
