...
def forward(
    ...
    if model.contextualizer_conf["contextualizer_type"] in CONTEXTUAL_HISTORY_ADAPTER_ENCODER:
        bias_vec, context_logit, context_prob = model.contextualizer(
            model_embed=encoder_out,
            context_idxs=contexts['blist'],
            context_ilens=contexts['ilens'],
            context_xphone_idxs=contexts['blist_xphone_mean'],
            context_history_idx=contexts['label_ctc'],
            context_history_ilens=contexts['label_ctc_ilens'],
        )
        idx, atten = model.contextualizer.batch_greedy_mono_decoding(
            contexts['label_ctc'], 
            context_prob, 
            context_prob
        )
    ...
    target      = contexts['label_ctc']
    logp        = torch.log(context_prob)[0].transpose(1, 0)
...
if __name__ == "__main__":
    ...
    model_conf = "./conf/contextual_adapter/conformer/tune_context_history_adapter.yaml"
    model_path = "./exp/asr_conformer/finetune_freeze_con_enc_context_history_gactc_tem_test_suffix/1epoch.pth"
    ...
    model.contextualizer.adapter.temperature = 10
