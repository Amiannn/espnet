import os
import json
import jieba
import numpy as np
import matplotlib.pyplot as plt
import seaborn as sns  # Seaborn for enhanced styling

from jiwer import cer, wer
from tqdm import tqdm

from pyscripts.utils.fileio import read_file, read_json, write_json
from pyscripts.utils.text_aligner import CheatDetector, align_to_index

# ------------------------------------------------------------------------------
# Seaborn setup with a lighter palette
# ------------------------------------------------------------------------------
sns.set_theme(
    style="whitegrid",
    palette="bright",  # Use a pastel palette for lighter colors
    font_scale=1.5,
    rc={
        "axes.unicode_minus": False,  # So negative signs work properly
        "figure.figsize": (12, 8)     # Default figure size
    }
)

plt.rcParams.update({'font.size': 22})
plt.rcParams["figure.dpi"] = 300          
plt.rcParams["axes.labelsize"] = 12       
plt.rcParams["axes.titlesize"] = 14       
plt.rcParams["legend.fontsize"] = 10      
plt.rcParams["xtick.labelsize"] = 10      
plt.rcParams["ytick.labelsize"] = 10      
plt.rcParams["font.family"] = "sans-serif"

# ------------------------------------------------------------------------------
# Paths and file names (unchanged)
# ------------------------------------------------------------------------------
TEST_REF_PATH  = './dump/raw/test/text'
TRAIN_REF_PATH = './data/S95/text'
DUMP_PATH      = "./exp/statistics/context_long_tailed"

TEST_RAREWORD_LIST  = './local/contextual/contexts/context_keywords_test.txt'
TEST_UTT_BLIST_PATH = './dump/raw/test/uttblist_idx_keywords'

# TEST_RAREWORD_LIST  = './local/contextual/contexts/context_f10_test.txt'
# TEST_UTT_BLIST_PATH = './dump/raw/test/uttblist_idx_f10'

# TEST_RAREWORD_LIST  = './local/contextual/contexts/context_f65536_test.txt'
# TEST_UTT_BLIST_PATH = './dump/raw/test/uttblist_idx_f65536'

HYP_BASELINE_PATH = (
    '../asr1/exp/asr_train_conformer_raw_en_bpe5000_sp_suffix/'
    'decode_asr_bs3_asr_model_valid.acc.ave_10best/test/text'
)
HYP_CASR_PATH = (
    './exp/asr_conformer/run_context_adapter_encoder_suffix/'
    'decode_asr_contextual_bs3_asr_model_valid.acc.ave_10best/test/text'
)
HYP_OUR_PATH  = (
    # './exp/asr_conformer/run_context_adapter_encoder_reweight0.8_suffix/'
    # 'decode_asr_contextual_bs3_asr_model_valid.acc.ave_10best/test/text'
    './exp/asr_conformer/run_context_adapter_encoder_iw_suffix/'
    'decode_asr_contextual_bs3_asr_model_valid.acc.ave_10best/test/text'
)

# ------------------------------------------------------------------------------
# Helper functions (unchanged except for color adjustments in plotting)
# ------------------------------------------------------------------------------

def smooth_data(data, kernel_size=20):
    """
    Smooth the input 1D data using a simple moving average.
    """
    kernel = np.ones(kernel_size) / kernel_size
    length = len(data)
    # Duplicate data in reverse, then convolve to handle boundary effects.
    extended = data + data[::-1]
    return np.convolve(extended, kernel, mode='same')[:length]

def get_word_frequency(ref_data, bwords, remove_non_existing=False):
    """
    Count the frequency of each word in 'bwords' according to the reference data.
    """
    freq_dict = {word: 0 for word in bwords}
    for line in tqdm(ref_data, desc="Frequency Counting"):
        if len(line) < 2:
            continue    
        _, *ref_words = line
        for w in ref_words:
            if w in freq_dict:
                freq_dict[w] += 1

    # Sort by frequency (descending) for consistency, then reconstruct dict
    sorted_pairs = sorted(((count, w) for w, count in freq_dict.items()), 
                          reverse=True)
    freq_dict    = {w: c for c, w in sorted_pairs}

    # Also track words with zero count, if needed
    zero_count_words = [w for w, c in freq_dict.items() if c == 0]
    if remove_non_existing:
        return freq_dict, zero_count_words
    return freq_dict

def compute_context_error(ref_data, hyp_data, blist_idxs, bwords):
    """
    Compute context errors (substitutions) for each target bword.
    """
    error_dict = {word: [] for word in bwords}
    for i in tqdm(range(len(ref_data)), desc="Context Error"):
        if len(ref_data[i]) < 2:
            continue 
        _, *ref_words = ref_data[i]
        _, *hyp_words = hyp_data[i]
        _, blist_idx  = blist_idxs[i]

        # Get the actual bwords that appear in the reference for this utterance
        blist         = [bwords[idx] for idx in blist_idx]

        # Align references & hypotheses at the token level
        chunks = align_to_index(ref_words, hyp_words)
        for chunk in chunks:
            wref, whyps, rindex, hindexis = chunk
            wref_clean = wref.replace('-', '')
            whyps_clean = ''.join(whyps).replace('-', '')
            # If bword is in the reference and it was misrecognized
            if (wref_clean in blist) and (wref_clean != whyps_clean):
                error_dict[wref_clean].append(whyps_clean)

    return error_dict

def compute_sorted_error_rate(error_dict, train_freq_dict, test_freq_dict):
    """
    Compute WER-based context error rate for each bword, sorted by training frequency.
    """
    sorted_error_rate = []
    for bword in train_freq_dict:
        train_occurrence = train_freq_dict[bword]
        test_occurrence  = test_freq_dict[bword]
        if test_occurrence == 0:
            error_rate = 0
        else:
            # Sum WER on each misrecognition, then average
            error_rate = sum(wer(bword, hyp_b) for hyp_b in error_dict[bword]) / test_occurrence
        sorted_error_rate.append(error_rate)
    return sorted_error_rate

def plot_error_rates(
    output_path, 
    train_occurrences, 
    baseline_errors, 
    casr_errors, 
    our_errors, 
    suffix=''
):
    """
    Plot and compare error rates for baseline, CASR, and "ours" approach,
    grouping by training frequency thresholds (shots).
    """

    # Shots dictionary: threshold => (max_frequency, color_for_shading)
    line_palette = sns.color_palette()
    shots_dict = {
        'many_shot':   [100, line_palette[2]],  # PaleGreen
        'medium_shot': [20,  line_palette[0]],  # LightBlue
        'few_shot':    [1,   line_palette[9]],  # PeachPuff
        'zero_shot':   [-1,  line_palette[6]],  # LightCoral
    }
    # Create figure and axis
    fig, ax = plt.subplots(figsize=(10, 6))

    # Highlight spans (for many/medium/few/zero-shot regions)
    start = 0
    end = 0
    for shot_type, (freq_threshold, color) in shots_dict.items():
        for i in range(start, len(train_occurrences)):
            # As soon as we find a word with a frequency < freq_threshold, break
            if train_occurrences[i] < freq_threshold:
                break
            end = i
        # To avoid multiple labels in legend
        if shot_type == 'many_shot':
            label_shading = "Many-shot ($n_{c} \geq 100$)"
        elif shot_type == 'medium_shot':
            label_shading = "Medium-shot ($100 > n_{c} \geq 20$)"
        elif shot_type == 'few_shot':
            label_shading = "Few-shot ($20 > n_{c} \geq 1$)"
        else:
            label_shading = "Zero-shot ($n_{c} = 0$)"

        ax.axvspan(xmin=start, xmax=end, facecolor=color, alpha=0.2, label=label_shading)
        start = end

    # Remove duplicate labels in the legend
    handles, labels = ax.get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    ax.legend(by_label.values(), by_label.keys(), loc="upper right", frameon=True)

    # X-axis setup
    x_vals = np.arange(len(baseline_errors))
    step   = max(1, len(baseline_errors) // 10)  # a reasonable step for ticks
    ax.set_xticks(x_vals[::step])
    ax.set_xticklabels(x_vals[::step], rotation=45)
    
    # Y-axis setup
    ax.set_ylim([0, 1.0])
    ax.set_xlim([0, len(baseline_errors)])
    ax.set_ylabel("Context Error Rate")
    ax.set_xlabel("Sorted Context Index")
    ax.grid(True, linestyle='--', alpha=0.5)

    # Smoothing factor and plotting lines
    smoothing_factor = 500

    # Define lighter line colors using Seaborn's palette
    baseline_color = shots_dict['medium_shot'][1]  # First color in the pastel palette
    casr_color     = shots_dict['few_shot'][1]  # Second color
    our_color      = shots_dict['zero_shot'][1]  # Third color

    # Baseline
    baseline_smoothed_short = smooth_data(baseline_errors, kernel_size=100)
    baseline_smoothed_long  = smooth_data(baseline_errors, kernel_size=smoothing_factor)
    ax.plot(x_vals, baseline_smoothed_short, color=baseline_color, linewidth=2, alpha=0.2)
    ax.plot(x_vals, baseline_smoothed_long,  color='white', linewidth=3, alpha=1)
    ax.plot(x_vals, baseline_smoothed_long,  color=baseline_color, linewidth=2, alpha=0.9, 
            label="Baseline")

    # CASR
    casr_smoothed_short = smooth_data(casr_errors, kernel_size=100)
    casr_smoothed_long  = smooth_data(casr_errors, kernel_size=smoothing_factor)
    ax.plot(x_vals, casr_smoothed_short, color=casr_color, linewidth=2, alpha=0.2)
    ax.plot(x_vals, casr_smoothed_long,  color='white', linewidth=3, alpha=1)
    ax.plot(x_vals, casr_smoothed_long,  color=casr_color, linewidth=2, alpha=0.9, 
            label="+ Contextual adapter")

    # Ours
    our_smoothed_short = smooth_data(our_errors, kernel_size=100)
    our_smoothed_long  = smooth_data(our_errors, kernel_size=smoothing_factor)
    ax.plot(x_vals, our_smoothed_short, color=our_color, linewidth=2, alpha=0.2)
    ax.plot(x_vals, our_smoothed_long,  color='white', linewidth=3, alpha=1)
    ax.plot(x_vals, our_smoothed_long,  color=our_color, linewidth=2, alpha=0.9, 
            label="+ Context-balanced adapter (ours)")

    ax.set_title("Long-tailed Performance", fontweight='bold', fontsize=12)
    ax.legend(loc="upper left")
    plt.tight_layout()

    # Save figure
    svg_path = os.path.join(output_path, f'word_plot_counts_{suffix}.svg')
    png_path = os.path.join(output_path, f'word_plot_counts_{suffix}.png')
    plt.savefig(svg_path, dpi=300)
    plt.savefig(png_path, dpi=300)
    plt.close(fig)
    print(f"Saved plots to:\n  {svg_path}\n  {png_path}")

# ------------------------------------------------------------------------------
# Main script (unchanged)
# ------------------------------------------------------------------------------
if __name__ == '__main__':
    # Read reference text
    ref_test_data  = read_file(TEST_REF_PATH, sp=' ')
    ref_train_data = read_file(TRAIN_REF_PATH, sp=' ')

    # Read hypotheses
    hyp_our_data       = read_file(HYP_OUR_PATH, sp=' ')
    hyp_casr_data      = read_file(HYP_CASR_PATH, sp=' ')
    hyp_baseline_data  = read_file(HYP_BASELINE_PATH, sp=' ')

    # Read list of contextual bwords and their indices
    test_blist_idxs = [
        [line[0], [int(x) for x in line[1:] if x != '']] 
        for line in read_file(TEST_UTT_BLIST_PATH, sp=' ')
    ]
    test_bwords = [
        line[0] 
        for line in read_file(TEST_RAREWORD_LIST, sp=',')
    ]

    # Get frequencies
    test_freq_dict, zero_count_bwords  = get_word_frequency(
        ref_test_data, test_bwords, remove_non_existing=True
    )
    train_freq_dict = get_word_frequency(ref_train_data, test_bwords)

    # Remove words with zero counts
    test_freq_dict_filtered  = {
        w: test_freq_dict[w] 
        for w in test_freq_dict 
        if w not in zero_count_bwords
    }
    train_freq_dict_filtered = {
        w: train_freq_dict[w] 
        for w in train_freq_dict 
        if w not in zero_count_bwords
    }

    # Compute error dictionaries
    error_dict_baseline = compute_context_error(ref_test_data, hyp_baseline_data, test_blist_idxs, test_bwords)
    error_dict_casr     = compute_context_error(ref_test_data, hyp_casr_data, test_blist_idxs, test_bwords)
    error_dict_our      = compute_context_error(ref_test_data, hyp_our_data, test_blist_idxs, test_bwords)

    # Compute sorted error rates
    baseline_error_rate = compute_sorted_error_rate(error_dict_baseline, 
                                                    train_freq_dict_filtered, 
                                                    test_freq_dict_filtered)
    casr_error_rate     = compute_sorted_error_rate(error_dict_casr, 
                                                    train_freq_dict_filtered, 
                                                    test_freq_dict_filtered)
    our_error_rate      = compute_sorted_error_rate(error_dict_our, 
                                                    train_freq_dict_filtered, 
                                                    test_freq_dict_filtered)

    # Prepare data for plotting
    occurrences = np.array(list(train_freq_dict_filtered.values()))
    print("Occurrences (Train Frequencies):", occurrences)

    # Plot the results
    plot_error_rates(
        DUMP_PATH,
        occurrences,
        baseline_error_rate,
        casr_error_rate,
        our_error_rate,
        suffix="slidespeech"
    )
